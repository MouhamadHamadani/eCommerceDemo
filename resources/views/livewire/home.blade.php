...
<div>
          ...
          <div class="relative">
            @if ($slide->description != null || $slide->title != null || $slide->link != null)
              <div class="absolute lg:w-1/2 h-full flex justify-center items-start flex-col gap-5 px-16">
                ...
                @endif
                @if ($slide->link != null)
                  <a href="{{ $slide->link }}" wire:navigate><button
                      class="bg-green-600 hover:bg-green-700 duration-300 text-white px-10 py-2 rounded-full">Shop
                      Now</button></a>
                @endif
              </div>
            @endif
  ...
  <div class="text-center">
    ...
    <div class="flex justify-center flex-wrap">
      @forelse($categories as $category)
        <a href="{{ url('products?category=' . $category->slug) }}" class="2xl:w-[20%] md:w-1/4 w-1/2 group"
          wire:navigate>
          <div class="relative group">
            <img src="{{ Storage::url('categories/' . $category->image) }}" class="w-full">
            <label
              class="absolute cursor-pointer right-4 md:top-1/2 top-1/3 w-1/2 text-3xl font-black text-white group-hover:text-shadow-lg shadow-black text-end duration-300 group-hover:right-7">{{ $category->name }}</label>
          </div>
        </a>
      @empty
  ...
  {{-- Discount Products --}}
  @livewire('top-discounted-products')
  {{-- Discount Products --}}
